...
export class IndexedDbService {
  ...
  createDatabase(name): Observable<any> {
  ...
  createSchemes(schema): Observable<any> {
    if (!this.db.isOpen()) {
      throw new Error('Database is not open');
    }
    
    this.db.version(1).stores(schema);
    return from(this.db.open()).pipe(
      map(() => {}),
      catchError(error => {
        console.error(error);
  ...
  getRows(databaseName: string, tableName: string): Observable<any[]> {
    console.log(`getrows ${databaseName} ${tableName}`);
  ...
  insertRow(
    ...
    return from(Dexie.getDatabaseNames()).pipe(
      ...
      map(() => {
        ...
        return this.db[tableName].put(row);
      })
    );
  }
